...
import { ActionTypes, getReviewsSuccess, getReviewsError, GetReviewsAction } from '../actions/reviews';
...
export const getReviewsEpic = (action$: Observable<GetReviewsAction>) =>
  action$.pipe(
    ...
    switchMap((action: GetReviewsAction) =>
      from(getReviewsFromApi(action.payload.traveledWith, action.payload.sortBy, action.payload.page)).pipe(
